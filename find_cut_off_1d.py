...
def main(python_path, w_dir, iter=5, input_file='cluster/rawAllx1000.json', cutoffs=range(5, 100, 5), seeds=[1],
    ...
    for i in cutoffs:
        ...
        for feature_index in range(len(features)):
            ...
            the_lambda = 'lambda x : ' + lambdas[feature_index]
            kde_name = os.path.join(w_dir, 'cuts/cut%02d_%s.kde' % (i, feature))
            configurations += ['-C', feature, kde_name, the_lambda]
            arguments = [
        ...
        for seed in seeds:
            output_file_name = os.path.join(w_dir, 'cuts/cut1d%02d-%d.o' % (i, seed))
            with open(output_file_name, 'w') as output_file:
                arguments = [
                    ...
                    '-t'] + task_ids + [
                    '--seed', str(seed),
                    '--save', os.path.join(w_dir, 'cuts/cut1d%02d-%d.json' % (i, seed))
                ]
                print(' '.join(arguments))
